...
testthat::test_that("handle NAs", {
  expect_true(
    "No_idea" %in%
      (test3 %>% spread_with_multiple_values(key, value, fill = "No_idea") %>% 
         pull(C)))
})
...
testthat::test_that("apply aggregate function", {
  expect_equal(
    test1 %>%
      spread_with_multiple_values(key, value, aggfunc = paste, 
                                  collapse = "-") %>%
      pull(C), 
    test1 %>% filter(key == "C") %>% pull(value) %>% paste(collapse = "-"))
  expect_equal(
    test1 %>%
      spread_with_multiple_values(key, value, aggfunc = str_c, 
                                  collapse = "-") %>%
      pull(C), 
...
testthat::test_that("drop is FALSE", {
  ...
  expect_equal(table2[-c(1:2),] %>% 
                  spread_with_multiple_values(type, count, drop = FALSE, 
                                              fill = 0),
                table2[-c(1:2),] %>% 
