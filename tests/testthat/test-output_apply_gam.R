...
testthat::test_that("Test output", {
  ...
  expect_true(all(basic_gam$output$em1 %in% c(-1, 0, 1),
                  basic_gam$output$em2 %in% c(-1, 0, 1)))
  expect_true(all(basic_gam_ys$output$em1 %in% c(-1, 0, 1),
                  basic_gam_ys$output$em2 %in% c(-1, 0, 1)))
  expect_true(all(corrected_gam$output$em1 %in% c(-1, 0, 1),
                  corrected_gam$output$em2 %in% c(-1, 0, 1)))
  expect_true(all(corrected_gam_ys$output$em1 %in% c(-1, 0, 1),
